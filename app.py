...
app.config["SECRET_KEY"] = os.environ.get(
    "SECRET_KEY",
    "contrato-mais-secret-key-2024-super-seguro"
)
...
# =========================
# ✅ ADMIN VIA ENV (SEM SENHA FIXA NO CÓDIGO)
# =========================
ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL", "admin@contratomais.com")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "Junior02199!")  # defina no Render
...
CORS(
    ...
    origins=[
        "http://localhost:5000",
        "http://127.0.0.1:5000",
        "https://contrato-p3r8.onrender.com",
    ],
)

# =========================
# ✅ EMAIL VIA ENV (NUNCA HARDCODE)
# =========================
EMAIL_CONFIG = {
    ...
    "use_tls": True if os.environ.get("SMTP_USE_TLS", "true").lower() == "true" else False,
}

# =========================
# ✅ SQLITE PERSISTENTE (RENDER DISK)
# =========================
# No Render, crie um Persistent Disk e monte em /var/data (ou outro path).
# Você pode definir DISK_PATH no Render com o mesmo mount path.
DISK_PATH = os.environ.get("DISK_PATH", "/var/data")
try:
    os.makedirs(DISK_PATH, exist_ok=True)
except Exception as e:
    # Se por algum motivo não der pra criar (permissão),
    # cai para DATA_DIR (mas isso pode ser efêmero no Render).
    logger.warning("Não foi possível criar DISK_PATH=%s (%s). Usando DATA_DIR=%s", DISK_PATH, e, DATA_DIR)
    DISK_PATH = DATA_DIR
    os.makedirs(DISK_PATH, exist_ok=True)

DATABASE = os.path.join(DISK_PATH, "contratos.db")
logger.info("DB em uso: %s", DATABASE)

# ✅ RESET CODE (se você removeu a zona de perigo do front, pode manter isso só pra admin interno)
RESET_CODE = os.environ.get("RESET_CODE", "19192425")
...
def criar_tabelas():
    ...
    # =========================
    # ✅ GARANTE ADMIN (CRIA OU ATUALIZA SENHA)
    # =========================
    cursor.execute("SELECT id FROM usuario WHERE email = ?", (ADMIN_EMAIL,))
    ...
    if not admin:
        cursor.execute(
            ...
            ("Administrador", ADMIN_EMAIL, senha_hash_admin),
        )
        logger.info("Admin criado: %s", ADMIN_EMAIL)
    else:
        cursor.execute(
            "UPDATE usuario SET senha_hash = ? WHERE email = ?",
            (senha_hash_admin, ADMIN_EMAIL),
        )
        logger.info("Senha do admin sincronizada via ENV: %s", ADMIN_EMAIL)
...
def criar_template_email(assunto, titulo, mensagem, tipo_notificacao=None, contrato=None):
    ...
    dashboard_link = f"{BASE_URL}/dashboard.html"
        ...
        <div style="max-width:700px;margin:0 auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 10px 25px rgba(0,0,0,.08)">
            ...
            <div style="padding:22px;">
                ...
                <div style="text-align:center;margin-top:18px;">
                    <a href="{dashboard_link}" style="background:{cor_primaria};color:white;padding:12px 22px;border-radius:999px;text-decoration:none;display:inline-block;font-weight:800;">
                        Acessar Dashboard
                    </a>
                </div>
            </div>
...
def enviar_email(destinatarios, assunto, corpo_html, corpo_texto=None):
    # Se não configurou email, não quebra o sistema
    if not EMAIL_CONFIG["sender_email"] or not EMAIL_CONFIG["sender_password"]:
        logger.warning("SMTP não configurado (SMTP_SENDER_EMAIL/SMTP_SENDER_PASSWORD vazios). Email não será enviado.")
        return False
...
if __name__ == "__main__":
    ...
    print(f"Servidor iniciado em: {BASE_URL}")
